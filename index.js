...
async function run() {
    try {
        ...
        const paymentCollection = client.db('cars').collection('bookingPaymentCollection');
        ...
        app.get('/payment/:id', async (req, res) => {
            ...
            const result = await bookingCars.findOne(query);
            res.send(result);
        })
        ...
        app.post('/payments', async(req, res)=>{
            ...
            const updatedDoc = {
                $set:{
                    paid:true,
                    transactionId : payment.transonId
                }
            }
            const updatedResult = await bookingCars.updateOne(filter, updatedDoc)
            res.send(result)
        })
    }
    finally {
